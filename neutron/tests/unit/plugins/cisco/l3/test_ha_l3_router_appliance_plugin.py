...
class HAL3RouterApplianceVMTestCase(
    ...
    def test_floatingip_multi_external_one_internal(self):
        with contextlib.nested(self.subnet(cidr="10.0.0.0/24"),
                               ...
                               self.subnet(cidr="12.0.0.0/24")) as (
                exs1, exs2, ins1):
            ...
            with contextlib.nested(
                    ...
                    self.port(subnet=ins1, fixed_ips=r2i_fixed_ips)) as (
                    r1, r2, r2i_port):
    ...
    def test_router_update_change_external_gateway_and_routes(self):
        with self.router() as router:
            r = router['router']
            with contextlib.nested(self.subnet(cidr='12.0.1.0/24'),
    ...
    def test_redundancy_router_routes_is_from_user_visible_router(self):
        ...
        with contextlib.nested(self.router(),
                               self.subnet(cidr='10.0.1.0/24')) as (router,
                                                                    subnet):
    ...
    def test_redundancy_router_routes_includes_user_visible_router(self):
        ...
        with contextlib.nested(self.router(),
                               ...
                               self.subnet(cidr='11.0.1.0/24')) as (
                router, subnet1, subnet2):
            with contextlib.nested(self.port(subnet=subnet1),
                                   self.port(subnet=subnet2)) as (port1,
                                                                  port2):
                r = router['router']
                p1 = port1['port']
                p2 = port2['port']
                updated_r = self._routes_update_prepare(
                    r['id'], None, p1['id'], routes1)['router']
